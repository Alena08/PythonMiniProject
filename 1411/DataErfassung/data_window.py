...
# person_window = tk.Tk()
# person_window.title("Complete your profile")
# person_window.geometry("500x400")
customFont= ("Helvetica", 20)

def open_person_data_window(master):
    """New window for enter Person data."""
    
    person_window = tk.Toplevel(master) 
    person_window.title("Person Data Entry") 
    ...
    image_frame = tk.Frame(person_window, width=50, height=50)
    ...
    for i,elemnt in enumerate(["Name","Gender","Eye Color","Height (cm)","Weight (kg)"]):
        tk.Label(person_window, text=f"{elemnt}:", font=customFont).grid(column=1, row=i, sticky="w")

    name_entry = tk.Entry(person_window, font=customFont)
    name_entry.grid(column=2, row=0, columnspan=2)

    gender = tk.StringVar()
    gender_combx = ttk.Combobox(person_window,width=20, textvariable=gender, font=customFont)
    gender_combx['values']=("Male", "Female")
    gender_combx.grid(column=2, row=1, columnspan=2)
    gender_combx.current(0)

    eyeColor = tk.StringVar()
    eyeColor_combx = ttk.Combobox(person_window,width=20, textvariable=eyeColor, font=customFont)
    eyeColor_combx['values']=("Brown","Black","Blue","Green","Red","Hony","Olive")
    eyeColor_combx.grid(column=2,row=2, columnspan=2)

    height_entry = tk.Entry(person_window, font=customFont)
    height_entry.grid(column=2, row=3, columnspan=2)

    weight_entry = tk.Entry(person_window, font=customFont)
    weight_entry.grid(column=2, row=4, columnspan=2)

    submit_btn = tk.Button(person_window, text="Submit",font=customFont, command=submit)
